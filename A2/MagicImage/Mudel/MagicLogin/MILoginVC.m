...
//    [resetBtn addTarget:self action:@selector(resetBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [visitorsBtn setTitle:@"Visitors Login" forState:UIControlStateNormal];
    [visitorsBtn setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
//    [visitorsBtn setBackgroundColor:RGB(235, 235, 242)];
    visitorsBtn.layer.masksToBounds = YES;
//    visitorsBtn.layer.cornerRadius = 15;
    visitorsBtn.titleLabel.font = [UIFont systemFontOfSize:14.0];
    [visitorsBtn addTarget:self action:@selector(visitorsBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
    [maskImgView addSubview:visitorsBtn];
    visitorsBtn.sd_layout
    .leftSpaceToView(maskImgView, 30)
    .rightSpaceToView(maskImgView, 30)
    .topSpaceToView(registerBtn, 15)
    .heightIs(35);
    
    UILabel * contentLabel = [[UILabel alloc] init];
    contentLabel.textColor = [UIColor whiteColor];
    contentLabel.font = LSFontSize(13, [UIScreen mainScreen].bounds.size.width);
    contentLabel.text = @"Register login App,  you can use the member rights and interests on all platforms. visitors login, will open account for the current device.";
    [maskImgView addSubview:contentLabel];
    contentLabel.sd_layout
    .leftSpaceToView(maskImgView, 40)
    .rightSpaceToView(maskImgView, 40)
    .topSpaceToView(visitorsBtn, 10)
    .autoHeightRatio(0);
...
- (void)loginBtnClick:(UIButton *)sender {
    ...
    [MIHttpTool Post:SHLogin parameters:@{@"user_name":self.usrName.text,@"password":self.usrPwd.text} success:^(id responseObject) {
        sender.enabled = YES;
        if ([responseObject[@"status"] integerValue] == 1) {
            [MBProgressHUD showTipMessageInWindow:@"Login Success"];
            [UserDefaults setObject:responseObject[@"data"][@"id"] forKey:@"user_id"];
            [UserDefaults setObject:self.usrName.text forKey:@"nickName"];
...
//    NSLog(@"=====注册=====");
    MIRegisterVC * regiVC = [[MIRegisterVC alloc] init];
    regiVC.block = ^(NSString * _Nonnull username, NSString * _Nonnull password) {
        self.usrName.text = username;
        self.usrPwd.text = password;
    };
    [self.navigationController pushViewController:regiVC animated:YES];
    
}
- (void)visitorsBtnClicked:(UIButton *)sender {
    sender.enabled = NO;
